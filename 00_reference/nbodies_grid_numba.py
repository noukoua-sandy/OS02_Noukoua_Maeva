...
def update_positions(dt: float):
    global system, BENCH_STEPS, STEP_TIMES
    ...
    if BENCH_STEPS is not None and len(STEP_TIMES) >= BENCH_STEPS:
        arr = np.array(STEP_TIMES)
        print(f"[BENCH_RENDER] steps={len(arr)}")
        print(f"[BENCH_RENDER] compute_mean_ms={arr.mean():.4f}")
        print(f"[BENCH_RENDER] compute_min_ms={arr.min():.4f}")
        print(f"[BENCH_RENDER] compute_max_ms={arr.max():.4f}")
        raise SystemExit(0)
